...
const struct iokey_port iokey_list[] = {
    {
        ...
        .key_type.one_io.port = TCFG_IOKEY_VOLDOWN_ONE_PORT,    //IO按键对应的引脚
        .key_value = 0,                                       //按键值
    },
...

	{
        ...
        .key_type.two_io.out_port = TCFG_IOKEY_NEXT_OUT_PORT,
        .key_type.two_io.in_port = TCFG_IOKEY_NEXT_IN_PORT,
        .key_value = 3,
    },

	{
        .connect_way = TCFG_IOKEY_PP_CONNECT_WAY,
        .key_type.two_io.out_port = TCFG_IOKEY_PP_OUT_PORT,
        .key_type.two_io.in_port = TCFG_IOKEY_PP_IN_PORT,
        .key_value = 4,
    }
};
...
static void board_power_wakeup_init(void)
{
   ...
   /* 注意：
    * power_wakeup_init()里面用到PORTA B C D的DIE、DIEH寄存器，
    * 操作了这些寄存器会导致mfi认证失败，
    * 原因不明
    * */
	return ;
